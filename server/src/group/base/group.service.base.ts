...
export class GroupServiceBase {
  ...
  async findUserGroups(
    parentId: string,
    args: Prisma.UserGroupFindManyArgs
  ): Promise<UserGroup[]> {
  ...
  async getGraduationType(parentId: string): Promise<GraduationType | null> {
    return this.prisma.group
      .findUnique({
        where: { id: parentId },
      })
  ...
  async getInstitute(parentId: string): Promise<Institute | null> {
    return this.prisma.group
      .findUnique({
        where: { id: parentId },
      })
  ...
  async getMarketProfile(parentId: string): Promise<MArketProfile | null> {
    return this.prisma.group
      .findUnique({
        where: { id: parentId },
      })
