/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { GroupUpdateManyWithoutGraduationTypesInput } from "./GroupUpdateManyWithoutGraduationTypesInput";
import { ValidateNested, IsOptional, IsString } from "class-validator";
import { Type } from "class-transformer";
import { InstituteTypeUpdateManyWithoutGraduationTypesInput } from "./InstituteTypeUpdateManyWithoutGraduationTypesInput";
@InputType()
class GraduationTypeUpdateInput {
  @ApiProperty({
    required: false,
    type: () => GroupUpdateManyWithoutGraduationTypesInput,
  })
  @ValidateNested()
  @Type(() => GroupUpdateManyWithoutGraduationTypesInput)
  @IsOptional()
  @Field(() => GroupUpdateManyWithoutGraduationTypesInput, {
    nullable: true,
  })
  groups?: GroupUpdateManyWithoutGraduationTypesInput;

  @ApiProperty({
    required: false,
    type: () => InstituteTypeUpdateManyWithoutGraduationTypesInput,
  })
  @ValidateNested()
  @Type(() => InstituteTypeUpdateManyWithoutGraduationTypesInput)
  @IsOptional()
  @Field(() => InstituteTypeUpdateManyWithoutGraduationTypesInput, {
    nullable: true,
  })
  instituteTypes?: InstituteTypeUpdateManyWithoutGraduationTypesInput;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  name?: string | null;
}
export { GraduationTypeUpdateInput };
